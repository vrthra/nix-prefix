...
#@ file=ftp://ftp.astron.com/pub/file/file-5.27.tar.gz
...
build/%/.extracted: static/% build/file.x/.build
	mkdir -p build/$*
...
build/file.x/.build: build/file.x/.extracted
...
$(nixpkgs_config)/config.nix:
	...
	cat etc/config.nix | sed -e 's#BASE#$(base)#g' >  $(nixpkgs_config)/config.nix

build/.nixconfig: $(addprefix build/,$(addsuffix .x/.build,$(src))) build/.nixpkgs $(nixpkgs_config)/config.nix
	env LD_LIBRARY_PATH="$(nix_boot_usr)/lib:$(nix_boot_usr)/lib64" \
		$(nix_boot_usr)/bin/nix-env -iA nixUnstable -f $(NIXPKGS)
	touch $@

build/.nixpkgs: $(addprefix build/,$(addsuffix .x/.build,$(src))) build/.pkgcheckout
	touch $@
...
$(nixprofile):
	ln -s $(nix_root)/var/nix/profiles/default $(nixprofile)
